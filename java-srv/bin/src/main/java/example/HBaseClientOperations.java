...
public class HBaseClientOperations {
    ...
    // https://stackoverflow.com/questions/74088595/how-to-create-a-table-in-hbase-using-java-api
    private TableDescriptorBuilder createColumns()
    {
        System.out.println("create columns entered.");
        TableDescriptorBuilder tableDescBuilder = TableDescriptorBuilder.newBuilder(table1);
        for(String family: new String[]{family1, family2}){
            ColumnFamilyDescriptorBuilder columnDescBuilder = ColumnFamilyDescriptorBuilder
                    .newBuilder(Bytes.toBytes(family))
                    .setCompressionType(Compression.Algorithm.SNAPPY)
                    .setDataBlockEncoding(DataBlockEncoding.FAST_DIFF);
            System.out.println("create column family: " + family);
            tableDescBuilder.setColumnFamily(columnDescBuilder.build());
            System.out.println("set column family: " + family);
        }
        System.out.println("create columns exited.");
    ...
    private void createTable(HBaseAdmin admin) throws IOException {
        ...
        // TableDescriptorBuilder tableDescBuilder = createColumns();
        // TableDescriptor desc = tableDescBuilder.build();
        // System.out.println("desc created.");
        HTableDescriptor desc = new HTableDescriptor(table1);
        desc.addFamily(new HColumnDescriptor(family1));
        desc.addFamily(new HColumnDescriptor(family2));
        admin.createTable(desc);
        System.out.println("create table exit.");
    }
